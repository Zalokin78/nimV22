...
const detectTouch = function () {
  //select all .button elements and store node array in buttons
  const buttons = document.getElementsByClassName("buttons");

  //this does the detection of touch capability and determine in boolean form
  const isTouch =
    !!("ontouchstart" in window) || window.navigator.msMaxTouchPoints > 0;
  //cycle through all nodes and add hover class
  if (!isTouch) {
